...
const GenerateSimilarQuestionOutputSchema = z.object({
    generatedQuestion: z.string().describe('The full generated question text in the specified format, including any new images.')
});
...
If the original question contains an image, generate a new placeholder image for the new question by creating a URL like 'https://placehold.co/WIDTHxHEIGHT.png' where WIDTH and HEIGHT are appropriate for the question. Include this new image markdown in the 'generatedQuestion' output field. For example: '![Alt text](https://placehold.co/400x300.png)'. If the original question has no image, do not generate one.

Provide the response in the following structured format. Do not add any extra text or formatting.
